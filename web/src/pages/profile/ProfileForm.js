...
export const ProfileForm = () => {
  ...
  const onProfileSubmit = (profileData) =>
    updateProfile(profileData)
      .then((resp) => new Promise((resolve) => setTimeout(() => resolve(resp), 1500)))
      .catch(error => {
        if (error.response) {
    ...
    <Form
      ...
      render={({ handleSubmit, submitError, submitting, submitSucceeded }) => (
        <form onSubmit={handleSubmit} className={classes.form}>
          ...
          { submitSucceeded && <PaddedAlert elevation={6} variant='filled' severity='success'>Profile updated</PaddedAlert>}
          { submitError && <PaddedAlert elevation={6} variant='filled' severity='error'>{submitError}</PaddedAlert>}
        </form>
      )}
    />
  )
